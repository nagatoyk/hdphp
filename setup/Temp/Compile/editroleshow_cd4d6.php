...
<div class="hd_setup">
    ...
    <h2>
        <a href="http://localhost/hdphp/setup/index.php?c=Rbac">返回安装首页</a>&nbsp;&nbsp;&nbsp;
        <a href="http://localhost/hdphp/setup/index.php?c=Rbac/showrole" target="_self">查看用户组(角色)</a>&nbsp;&nbsp;&nbsp;
        <a href="javascript:void(0)" onclick="window.close();return false;">关闭</a>
        &nbsp;<a href="http://localhost/hdphp/setup/index.php?c=Rbac/lock" >锁定SETUP应用</a>
    </h2>
</div>
<form method="post" name="form1" id="form1" action="<?php echo U(editrole);?>">
    <input type="hidden" name="rid" value="<?php echo $role['rid'];?>"/>
    <input type="hidden" name="pid" value="<?php echo $role['pid'];?>"/>
    <div class="setup">
        <dl>
            <dt>添加用户角色（用户组）</dt>
            <dd>
                <table>
                    <tr><td width="200">角色(组)名称:</td>
                        <td>
                            <input type="text" name="rname" id="rname" value="<?php echo $role['rname'];?>"/><span>必须为英文字母表示</span>
                        </td>
                    </tr>
                    <tr><td width="200">角色描述:</td>
                        <td>
                            <input type="text" name="title" id="title" value="<?php echo $role['title'];?>"/><span>中文文字说明</span>
                        </td>
                    </tr>
                    <tr>
                        <td width="200">是否开启角色</td>
                        <td><input type="radio" name="state" id="state" value="1"  style="width:20px;" <?php if($role['state']==1){?>checked="checked"<?php }?>/> 开启
                            <input type="radio" name="state" id="state" value="0"  style="width:20px;" <?php if($role['state']==0){?>checked="checked"<?php }?>/> 关闭</td>
                    </tr>
